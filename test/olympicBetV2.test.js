...
!developmentChains.includes(network.name)
  ...
  : describe("OlympicBetV2", async () => {
      ...
      beforeEach(async () => {
        ...
        const OlympiBetV1 = await ethers.getContractFactory("OlympicBet");
        const OlympiBetV2 = await ethers.getContractFactory("OlympicBetV2");
        const olympicBeV1 = await upgrades.deployProxy(OlympiBetV1, []);
        olympicBet = await upgrades.upgradeProxy(olympicBeV1, OlympiBetV2);
      });
      ...
      describe("Withdraw GAS", function () {
        ...
        it("Should withdraw GAS successfully", async () => {
          const withdrawAmount = ethers.parseEther("5");
          const deployerBalanceBefore = await a.provider.getBalance(
            deployer.address
          );
          const contractBalanceBefore = await a.provider.getBalance(
            olympicBet.target
          );
          const tx = await olympicBet
            .connect(deployer)
            .withdrawGAS(withdrawAmount);
          const receipt = await tx.wait();
          const gasCost = BigInt(receipt.gasUsed) * BigInt(receipt.gasPrice);
          const deployerBalanceAfter = await a.provider.getBalance(
            deployer.address
          );
          const contractBalanceAfter = await a.provider.getBalance(
            olympicBet.target
          );
          assert.equal(
        ...
        it("Should revert if not enough balance", async () => {
          ...
          await expect(
            olympicBet.connect(deployer).withdrawGAS(withdrawAmount)
          ).to.be.revertedWith("Not enough GAS to withdraw");
        });
      });
    });
